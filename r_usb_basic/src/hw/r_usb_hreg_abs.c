...
void usb_hstd_set_hub_port (usb_utr_t *ptr, uint16_t addr, uint16_t upphub, uint16_t hubport)
{
    ...
    {
        HW_USB_HRmwDevadd(ptr, addr, (upphub|hubport), (uint16_t)(USB_UPPHUB | USB_HUBPORT));
    }
...
uint16_t usb_hstd_chk_attach (usb_utr_t *ptr, uint16_t port)
{
    ...
    if ((uint16_t) (buf[1] & USB_RHST) == USB_UNDECID)
    {
        if ((buf[0] & USB_LNST) == USB_FS_JSTS)
        {
            ...
            usb_hstd_set_hse(ptr, port, g_usb_cstd_hs_enable[ptr->ip]);
...
void usb_hstd_read_lnst (usb_utr_t *ptr, uint16_t port, uint16_t *buf)
{
    do
    {
        buf[0] = HW_USB_ReadSyssts(ptr, port);

        /* 30ms wait */
        usb_cpu_delay_xms((uint16_t) 30);
        buf[1] = HW_USB_ReadSyssts(ptr, port);
    ...
    buf[1] = HW_USB_ReadDvstctr(ptr, port);
...
void usb_hstd_chk_sof (usb_utr_t *ptr, uint16_t port)
{
    ...
    do
    {
        ...
        buf = HW_USB_ReadSyssts(ptr, port);
    }
...
void usb_hstd_resume_process (usb_utr_t *ptr, uint16_t port)
{
    ...
    /* RESUME=1, RWUPE=0 */
    HW_USB_RmwDvstctr(ptr, port, USB_RESUME, (USB_RESUME | USB_RWUPE));
...
uint8_t *usb_hstd_write_fifo (usb_utr_t *ptr, uint16_t count, uint16_t pipemode, uint8_t *write_p)
{
    ...
    {
        /* 16bit access */
        HW_USB_WriteFifo16(ptr, pipemode, *((uint16_t *) write_p));
    ...
    if ((count & (uint16_t) 0x0001u) != 0u)
    {
        ...
        /* FIFO write */
        HW_UBS_WriteFifo8(ptr, pipemode, *write_p);
...
else if (ptr->ip == USB_USBIP_1)
{
    ...
    {
        /* 16bit access */
        HW_USB_WriteFifo32(ptr, pipemode, *((uint32_t *)write_p));
    ...
    /* Return FIFO access width */
    HW_USB_SetMbw(ptr, pipemode, USB_MBW_32);
}
...
uint8_t *usb_hstd_read_fifo (usb_utr_t *ptr, uint16_t count, uint16_t pipemode, uint8_t *read_p)
{
    ...
    {
        /* 16bit FIFO access */
        *(uint16_t *) read_p = HW_USB_ReadFifo16(ptr, pipemode);
    ...
    {
        /* 16bit FIFO access */
        odd_byte_data_temp = HW_USB_ReadFifo16(ptr, pipemode);
...
else if (ptr->ip == USB_USBIP_1)
{
    ...
    {
        /* 32bit FIFO access */
        *(uint32_t *)read_p= HW_USB_ReadFifo32(ptr, pipemode);
    ...
    {
        /* 32bit FIFO access */
        odd_byte_data_temp = HW_USB_ReadFifo32(ptr, pipemode);
        /* Condition compilation by the difference of the little endian */
        do
        {
...
void usb_hstd_forced_termination (usb_utr_t *ptr, uint16_t pipe, uint16_t status)
{
    ...
    buffer = HW_USB_ReadFifoctr(ptr, USB_CUSE);
    ...
    if (USB_NULL != g_usb_hstd_pipe[ptr->ip][pipe])
    {
        ...
        g_usb_hstd_pipe[ptr->ip][pipe]->tranlen = g_usb_hstd_data_cnt[ptr->ip][pipe];
        g_usb_hstd_pipe[ptr->ip][pipe]->status = status;
        g_usb_hstd_pipe[ptr->ip][pipe]->pipectr = HW_USB_ReadPipectr(ptr, pipe);
        g_usb_hstd_pipe[ptr->ip][pipe]->errcnt = (uint8_t) g_usb_hstd_ignore_cnt[ptr->ip][pipe];
        g_usb_hstd_pipe[ptr->ip][pipe]->ipp = ptr->ipp;
        g_usb_hstd_pipe[ptr->ip][pipe]->ip = ptr->ip;
        if (USB_NULL != (g_usb_hstd_pipe[ptr->ip][pipe]->complete))
        {
            (g_usb_hstd_pipe[ptr->ip][pipe]->complete)(g_usb_hstd_pipe[ptr->ip][pipe], 0, 0);
        }
