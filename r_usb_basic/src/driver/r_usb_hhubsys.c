...
/* Condition compilation by the difference of USB function */
    #if USB_NUM_USBIP == 2
uint16_t g_usb_shhub_class_seq[USB_NUM_USBIP] =
{   USB_SEQ_0, USB_SEQ_0};
uint16_t g_usb_shhub_init_seq[USB_NUM_USBIP] =
{   USB_SEQ_0, USB_SEQ_0};
uint16_t g_usb_shhub_init_port[USB_NUM_USBIP] =
{   USB_HUB_P1, USB_HUB_P1};
uint16_t g_usb_shhub_event_seq[USB_NUM_USBIP] =
{   USB_SEQ_0, USB_SEQ_0};
uint16_t g_usb_shhub_event_port[USB_NUM_USBIP] =
{   USB_HUB_P1, USB_HUB_P1};
uint16_t g_usb_shhub_attach_seq[USB_NUM_USBIP] =
{   USB_SEQ_0, USB_SEQ_0};
uint16_t g_usb_shhub_reset_seq[USB_NUM_USBIP] =
{   USB_SEQ_0, USB_SEQ_0};
uint16_t g_usb_shhub_state[USB_NUM_USBIP] =
{   USB_SEQ_0, USB_SEQ_0};
uint16_t g_usb_shhub_info[USB_NUM_USBIP] =
{   USB_SEQ_0, USB_SEQ_0};
uint16_t g_usb_shhub_hub_addr[USB_NUM_USBIP] =
{   USB_SEQ_0, USB_SEQ_0};
uint16_t g_usb_shhub_process[USB_NUM_USBIP] =
{   USB_SEQ_0, USB_SEQ_0};
...
uint8_t *g_usb_shhub_device_table[USB_NUM_USBIP];
uint8_t *g_usb_shhub_config_table[USB_NUM_USBIP];
uint8_t *g_usb_shhub_interface_table[USB_NUM_USBIP];
...
USB_NULL | USB_BFREOFF | USB_CFG_DBLBOFF | USB_CFG_CNTMDOFF | USB_CFG_SHTNAKOFF | USB_NULL | USB_NULL,
    ...
    #if USB_NUM_USBIP == 2
        {
            /* PIPE9 Definition */
            USB_PIPE9,
            USB_NULL | USB_BFREOFF | USB_CFG_DBLBOFF | USB_CFG_CNTMDOFF | USB_CFG_SHTNAKOFF
            | USB_NULL | USB_NULL,
            (uint16_t)USB_BUF_SIZE(64u) | USB_BUF_NUMB(7u),
            USB_NULL,
            USB_NULL,
...
/*extern uint16_t    HW_USB_HReadDevadd(usb_utr_t *ptr, uint16_t devadr);*/
...
void usb_hhub_registration (usb_utr_t *ptr, usb_hcdreg_t *callback)
{
    ...
    if (USB_NULL == callback)
    {
        ...
        driver.tpl = (uint16_t*) &g_usb_hhub_tpl[0];

        /* Pipe Information Table Define address */
        driver.pipetbl = (uint16_t*) &g_usb_hhub_def_ep_tbl[ptr->ip];
    }
    else
    {
        /* Target peripheral list */
...
uint16_t usb_hhub_get_hub_information (usb_utr_t *ptr, uint16_t hubaddr, usb_cb_t complete)
{
    ...
    g_usb_shhub_ctrl_mess[ptr->ip].tranadr = (void*) &g_usb_hhub_descriptor[ptr->ip][0];
    g_usb_shhub_ctrl_mess[ptr->ip].tranlen = (uint32_t) g_usb_shhub_class_request[ptr->ip][3];
    g_usb_shhub_ctrl_mess[ptr->ip].setup = g_usb_shhub_class_request[ptr->ip];
...
uint16_t usb_hhub_get_port_information (usb_utr_t *ptr, uint16_t hubaddr, uint16_t port, usb_cb_t complete)
{
    ...
    /* Port GetStatus */
    g_usb_shhub_ctrl_mess[ptr->ip].keyword = USB_PIPE0;
    g_usb_shhub_ctrl_mess[ptr->ip].tranadr = (void*) &g_usb_hhub_data[ptr->ip][hubaddr][0];
    g_usb_shhub_ctrl_mess[ptr->ip].tranlen = (uint32_t) g_usb_shhub_class_request[ptr->ip][3];
    g_usb_shhub_ctrl_mess[ptr->ip].setup = g_usb_shhub_class_request[ptr->ip];
    g_usb_shhub_ctrl_mess[ptr->ip].segment = USB_TRAN_END;
    g_usb_shhub_ctrl_mess[ptr->ip].complete = complete;
...
static void usb_hhub_enumeration (usb_clsinfo_t *ptr)
{
    ...
    checkerr = ptr->result;
    table[0] = (uint16_t*) g_usb_shhub_device_table[ptr->ip];
    table[1] = (uint16_t*) g_usb_shhub_config_table[ptr->ip];
    table[2] = (uint16_t*) g_usb_shhub_interface_table[ptr->ip];
    ...
    switch (g_usb_shhub_class_seq[ptr->ip])
    {
        ...
        case USB_SEQ_1 :
            ...
            if (USB_OK == retval)
            {
                string = g_usb_shhub_device_table[ptr->ip][15];
        ...
        case USB_SEQ_5 :
            ...
            switch (g_usb_shhub_spec[ptr->ip])
            {
                case USB_FSHUB : /* Full Speed Hub */
                    ...
                    {
                        retval = usb_hhub_pipe_info(ptr, (uint8_t *) g_usb_shhub_interface_table[ptr->ip],
                                g_usb_shhub_index[ptr->ip], g_usb_shhub_speed[ptr->ip],
                                (uint16_t) g_usb_shhub_config_table[ptr->ip][2]);
                        if (USB_ERROR == retval)
                        {
        ...
        switch (g_usb_shhub_attach_seq[ptr->ip])
        {
            ...
            case USB_SEQ_1 :
                ...
                if (0 != devaddr)
                {
                    ...
                    buffer = HW_USB_HReadDevadd(ptr, devaddr);
                    do
                    {
...
static void usb_hhub_check_class (usb_utr_t *ptr, uint16_t **table)
{
    ...
    g_usb_shhub_device_table[ptr->ip] = (uint8_t*) (table[0]);
    g_usb_shhub_config_table[ptr->ip] = (uint8_t*) (table[1]);
    g_usb_shhub_interface_table[ptr->ip] = (uint8_t*) (table[2]);
    *table[3] = USB_OK;
...
static void usb_hhub_trans_start (usb_utr_t *ptr, uint16_t hubaddr, uint32_t size, uint8_t *table, usb_cb_t complete)
{
    ...
    g_usb_shhub_data_mess[ptr->ip][hubaddr].tranadr = table;
    g_usb_shhub_data_mess[ptr->ip][hubaddr].tranlen = size;
    g_usb_shhub_data_mess[ptr->ip][hubaddr].setup = 0;
