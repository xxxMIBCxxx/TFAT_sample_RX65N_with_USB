 ...
 ******************************************************************************/
extern void usb_registration (usb_ctrl_t *ctrl, usb_cfg_t *cfg);
...
uint16_t usb_pstd_chk_vbsts (void)
{
    ...
    do
    {
        buf1 = HW_USB_ReadIntsts();
        usb_cpu_delay_1us((uint16_t) 10);
        buf2 = HW_USB_ReadIntsts();
        usb_cpu_delay_1us((uint16_t) 10);
        buf3 = HW_USB_ReadIntsts();
...
void usb_pdriver_init (usb_ctrl_t *ctrl, usb_cfg_t *cfg)
{
    ...
    for (i = USB_PIPE0; i <= USB_MAX_PIPE_NO; i++)
    {
        ...
        g_usb_pstd_pipe[i] = (usb_utr_t *) USB_NULL;
    }
    ...
    usb_registration(ctrl, cfg); /* Class Registration */
    #endif  /* defined(USB_CFG_PCDC_USE)||defined(USB_CFG_PHID_USE)||defined(USB_CFG_PMSC_USE)||defined(USB_CFG_PVND_USE) */
