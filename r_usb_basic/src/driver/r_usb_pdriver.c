...
/*uint16_t        g_usb_pstd_intsts0; *//* INTSTS0 */
...
usb_er_t usb_pstd_set_submitutr (usb_utr_t * utrmsg)
{
    ...
    g_usb_pstd_pipe[pipenum] = utrmsg;
...
uint16_t usb_pstd_get_interface_num (uint16_t con_num)
{
    ...
    num_if = *(uint8_t *) (g_usb_pstd_driver.configtbl + USB_DEV_B_NUM_INTERFACES);
...
uint16_t usb_pstd_get_alternate_num (uint16_t con_num, uint16_t int_num)
{
    ...
    ptr = g_usb_pstd_driver.configtbl;
    i = ptr[0];
    /* Interface descriptor[0] */
    ptr = (uint8_t *) ((uint32_t) ptr + ptr[0]);
    length = (uint16_t) (*(uint8_t *) ((uint32_t) g_usb_pstd_driver.configtbl + (uint16_t) 2u));
    length |= (uint16_t) ((uint16_t) (*(uint8_t *) ((uint32_t) g_usb_pstd_driver.configtbl + (uint16_t) 3u)) << 8u);
...
uint16_t usb_pstd_chk_remote (void)
{
    ...
    atr = *(uint8_t *) ((uint32_t) g_usb_pstd_driver.configtbl + (uint32_t) 7u);
...
uint8_t usb_pstd_get_current_power (void)
{
    ...
    tmp = *(uint8_t *) ((uint32_t) g_usb_pstd_driver.configtbl + (uint32_t) 7u);
    ...
    switch (pipe_number)
    {
        ...
        case USB_PERIPIPE :
            ...
            for (ep = USB_EP1; ep <= USB_MAX_EP_NO; ++ep)
            {
                for (dir = 0; dir < 2; dir++)
                {
                    if (g_usb_pstd_eptbl_index[dir][ep] != USB_ERROR)
                    {
