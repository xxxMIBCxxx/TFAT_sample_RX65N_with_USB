...
#ifndef __R_USB_EXTERN_H__
    #define __R_USB_EXTERN_H__
...
extern usb_utr_t *g_usb_pstd_pipe[USB_MAX_PIPE_NO + 1u];
extern uint32_t g_usb_pstd_data_cnt[USB_MAX_PIPE_NO + 1u]; /* PIPEn Buffer counter */
extern uint8_t *g_usb_pstd_data_ptr[USB_MAX_PIPE_NO + 1u]; /* PIPEn Buffer pointer(8bit) */
    #endif  /* (USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI */

    #if ( (USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST )
extern usb_utr_t *g_usb_hstd_pipe[][USB_MAX_PIPE_NO + 1u]; /* Message pipe */
extern uint8_t *g_usb_hstd_data_ptr[][USB_MAX_PIPE_NO + 1u]; /* PIPEn Buffer pointer(8bit) */
extern uint32_t g_usb_hstd_data_cnt[][USB_MAX_PIPE_NO + 1u]; /* PIPEn Buffer counter */
extern uint16_t g_usb_cstd_hs_enable[]; /* Hi-speed enable */
...
void usb_h_registration (usb_utr_t *ptr);
...
uint16_t usb_hhub_GetHubInformation (usb_utr_t *ptr, uint16_t hubaddr, usb_cb_t complete);
uint16_t usb_hhub_GetPortInformation (usb_utr_t *ptr, uint16_t hubaddr, uint16_t port, usb_cb_t complete);
...
usb_er_t usb_cstd_WaiMsg (uint8_t id, usb_msg_t* mess, usb_tm_t tm);
...
void usb_pstd_set_feature_function (void);
    #endif
/* (USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI */
