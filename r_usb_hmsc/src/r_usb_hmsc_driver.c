...
extern void usb_hstd_dummy_function (usb_utr_t *ptr, uint16_t data1, uint16_t data2);
extern uint32_t usb_ghstd_pipe_receive_length[USB_MAXPIPE_NUM + 1];
*/
...
uint8_t *g_usb_hmsc_device_table[USB_NUM_USBIP];
uint8_t *g_usb_hmsc_config_table[USB_NUM_USBIP];
uint8_t *g_usb_hmsc_interface_table[USB_NUM_USBIP];
...
static uint16_t usb_hmsc_csw_err_roop[USB_NUM_USBIP];

uint16_t usb_shmsc_data_stall_seq[USB_NUM_USBIP];
uint16_t g_usb_hmsc_speed[USB_NUM_USBIP];

uint8_t drive_search_que[USB_MAXSTRAGE];
uint8_t drive_search_que_cnt;
uint8_t drive_search_lock;


const uint16_t usb_hmsc_devicetpl[] =
{
  ...
  { /* IP No.0 */
    {
        /* MSC Device 1 */
        {
            ...
            USB_BUF_SIZE(1024)|USB_BUF_NUMB( 8),    /* PIPEBUF(for USBA USE) */
            USB_NULL,   /* PIPEMAXP */
            USB_NULL,   /* PIPEPERI */
...
static void usb_hmsc_check_result (usb_utr_t *mess, uint16_t, uint16_t);
...
static void usb_hmsc_drive_complete (usb_utr_t *ptr, uint16_t addr, uint16_t data2);

/*uint16_t usb_hmsc_smp_pipe_info (usb_utr_t *ptr, uint8_t *table, uint16_t msgnum, uint16_t speed, uint16_t length);*/
static void usb_hmsc_class_check_result (usb_utr_t *mess, uint16_t, uint16_t);
...
static void usb_hmsc_registration (usb_utr_t *ptr);
...
void usb_hmsc_task (void)
{
    ...
    /* Receive message */
    err = R_USB_RCV_MSG(USB_HMSC_MBX, (usb_msg_t** )&pmess);
    if (USB_OK != err)
    {
        return;
    }
    ...
    err = R_USB_REL_BLK(USB_HMSC_MPL, (usb_mh_t )pmess);
    if (USB_OK != err)
    {
...
static void usb_hmsc_class_check (usb_utr_t *mess)
{
    ...
    switch (g_usb_hmsc_init_seq[mess->ip])
    {
        case USB_SEQ_0 :
            ...
            if (USB_IFCLS_MAS != g_usb_hmsc_interface_table[mess->ip][USB_IF_B_INTERFACECLASS])
            {
            ...
            g_usb_hmsc_tmp_sub_class[mess->ip] = g_usb_hmsc_interface_table[mess->ip][USB_IF_B_INTERFACESUBCLASS];
            ...
            if (USB_BOTP == g_usb_hmsc_interface_table[mess->ip][USB_IF_B_INTERFACEPROTOCOL])
            {
            ...
            if (g_usb_hmsc_interface_table[mess->ip][USB_IF_B_NUMENDPOINTS] < 2)
            {
        ...
        case USB_SEQ_1 :
            ...
            {
                /* Send GetDescriptor(Stirng) */
                iproduct = g_usb_hmsc_device_table[mess->ip][USB_DEV_I_PRODUCT];
        ...
        case USB_SEQ_1 : /* Check CBW and Send Data */
            ...
            else /* Success */
            {
                switch (usb_shmsc_process[mess->ip])
                {
                    ...
                    break;
                    case USB_MSG_HMSC_DATA_OUT :
                        usb_hmsc_send_data(mess, side, pbuff, size);
                        usb_shmsc_data_seq[mess->ip]++;
                    break;
                    case USB_MSG_HMSC_DATA_IN :
        ...
        case USB_SEQ_4 : /* Check CSW */
            ...
            switch (hmsc_retval)
            {
                case USB_HMSC_OK : /* Success */
                    if (usb_hmsc_csw_err_roop[mess->ip] == USB_ON)
                    {
                        usb_hmsc_csw_err_roop[mess->ip] = USB_OFF;
                ...
                case USB_HMSC_CSW_ERR :
                    ...
                    usb_hmsc_csw_err_roop[mess->ip] = USB_ON;
...
/******************************************************************************
 ...
 : uint16_t string          : pdescriptor index
...
static uint16_t usb_hmsc_get_string_desc (usb_utr_t *ptr, uint16_t addr, uint16_t string, usb_cb_t complete)
{
    ...
    g_usb_hmsc_class_control[ptr->ip].tranadr = (void *) g_usb_hmsc_class_data[ptr->ip];
    ...
    g_usb_hmsc_class_control[ptr->ip].setup = (uint16_t *) &string_setup[ptr->ip];
...
static uint16_t usb_hmsc_send_cbw (usb_utr_t *ptr, uint16_t side)
{
    ...
    usb_hmsc_trans_data[ptr->ip][side].tranadr = (void*) &g_usb_hmsc_cbw[ptr->ip][side];
    ...
    usb_hmsc_trans_data[ptr->ip][side].setup = 0;
    ...
    usb_hstd_ChangeDeviceState(ptr, (usb_cb_t) &usb_hstd_dummy_function, toggle, g_usb_hmsc_out_pipe[ptr->ip][side]);
...
static uint16_t usb_hmsc_get_data (usb_utr_t *ptr, uint16_t side, uint8_t *buff, uint32_t size)
{
    ...
    /* Transfer data address */
    usb_hmsc_receive_data[ptr->ip][side].tranadr = (void*) buff;
    ...
    /* Not control transfer */
    usb_hmsc_receive_data[ptr->ip][side].setup = 0;
    ...
    usb_hstd_ChangeDeviceState(ptr, (usb_cb_t) &usb_hstd_dummy_function, toggle, g_usb_hmsc_in_pipe[ptr->ip][side]);
...
static uint16_t usb_hmsc_send_data (usb_utr_t *ptr, uint16_t side, uint8_t *buff, uint32_t size)
{
    ...
    /* Transfer data address */
    usb_hmsc_trans_data[ptr->ip][side].tranadr = (void*) buff;
...
static uint16_t usb_hmsc_get_csw (usb_utr_t *ptr, uint16_t side)
{
    ...
    /* Transfer data address */
    usb_hmsc_receive_data[ptr->ip][side].tranadr = (void*) &g_usb_hmsc_csw[ptr->ip][side];
...
static usb_er_t usb_hmsc_clear_stall (usb_utr_t *ptr, uint16_t pipe, usb_cb_t complete)
{
    ...
    err = usb_hstd_ChangeDeviceState(ptr, complete, USB_DO_CLR_STALL, pipe);
...
static void usb_hmsc_detach (usb_utr_t *ptr, uint16_t addr, uint16_t data2)
{
    ...
    if (drive_search_lock == addr)
    {
        drive_search_lock = USB_FALSE;
    }

    que_cnt = drive_search_que_cnt;
    for (i = 0; i < que_cnt; i++)
    {
        if (drive_search_que[i] == addr)
        {
            drive_search_que[i] = USB_FALSE;
            for (j = i; j < que_cnt; j++)
            {
                drive_search_que[j] = drive_search_que[j + 1];
            }
            drive_search_que_cnt--;
        }
    }

    side = R_USB_HmscRefDrvno(addr);
...
static void usb_hmsc_drive_complete (usb_utr_t *ptr, uint16_t addr, uint16_t data2)
{
    ...
    que_cnt = drive_search_que_cnt;
    if (drive_search_que_cnt)
    {
        drive_search_que[0] = USB_NULL;
        for (i = 0; i < que_cnt; i++)
        {
            drive_search_que[i] = drive_search_que[i + 1];
        }
        drive_search_que_cnt--;
    }
    drive_search_lock = USB_FALSE;
...
void usb_hmsc_registration (usb_utr_t *ptr)
{
    ...
    driver.tpl = (uint16_t*) &usb_hmsc_devicetpl; /* Target peripheral list. */
    ...
    {
        driver.pipetbl = (uint16_t*) &g_usb_hmsc_tmp_ep_tbl[ptr->ip][0]; /* Pipe def. table address. */
...
void usb_hmsc_message_retry (uint16_t id, usb_utr_t *mess)
{
    ...
    if ( USB_HSTRG_MBX == id)
    {
        err = R_USB_PGET_BLK(USB_HSTRG_MPL, &pblf);
    }
    if (USB_OK == err)
    {
        ...
        /* Send message */
        err = R_USB_SND_MSG(id, (usb_msg_t* )pblf);
        if (USB_OK != err)
        {
...
void usb_class_task (void)
{
    ...
    if (USB_FALSE == drive_search_lock)
    {
        if (drive_search_que_cnt > 0)
        {
            drive_search_lock = drive_search_que[0];

            utr.ip = USB_IP0;
            if ((drive_search_lock & USB_IP_MASK) == USBA_ADDRESS_OFFSET)
            {
                utr.ip = USB_IP1;
            }

            addr = drive_search_lock & USB_ADDRESS_MASK;
...
void usb_hmsc_strg_cmd_complete (usb_utr_t *mess, uint16_t devadr, uint16_t data2)
{
    ...
    if ( USB_DATA_OK == mess->status) /* Check usb receive status */
    {
