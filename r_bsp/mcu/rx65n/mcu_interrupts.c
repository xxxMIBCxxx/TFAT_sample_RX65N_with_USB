...
*         : 01.12.2015 0.40(WS2) Change the 362 lines of code.
*                                (From BSP_INT_SRC_BL1_RIIC2_EEI2 to BSP_INT_SRC_BL1_SCI9_ERI9.)
*                                Change the 791 lines of code.(From ICU.GRPAL0.BIT.IS11 to ICU.GRPAL0.BIT.IS12.)
*                                Change the 796 lines of code.(From ICU.GRPAL0.BIT.IS12 to ICU.GRPAL0.BIT.IS13.)
*         : 04.12.2015 0.40(WS3) Change the name of the BL2 Group Interrupts.
*                                (From BSP_INT_SRC_BL0_SDSI_SDIOI to BSP_INT_SRC_BL2_SDSI_SDIOI)
*                                 Change the name of the BL0 Group Interrupts.
...
bsp_int_err_t bsp_interrupt_group_enable_disable (bsp_int_src_t vector, bool enable, uint32_t ipl)
{
    ...
    else if ((vector >= BSP_INT_SRC_BL1_SDHI_CDETI) && (vector <= BSP_INT_SRC_BL1_SCI9_ERI9))
    {
        /* Group BL1. */
        if (true == enable)
        {
    ...
    else if (vector == BSP_INT_SRC_AL1_EDMAC0_EINT0)
    {
        /* Group AL1. */
        if (true == enable)
        {
