...
Macro definitions
***********************************************************************************************************************/
#if defined(BSP_MCU_RX231) || defined(BSP_MCU_RX64M) || defined(BSP_MCU_RX71M) || defined(BSP_MCU_RX23T) || defined(BSP_MCU_RX65N)
#define CPU_CYCLES_PER_LOOP        4	  // Known number of CPU cycles required to execute the delayWait() loop
#else
#define CPU_CYCLES_PER_LOOP        5
#endif

#define OVERHEAD_CYCLES            205 // R_BSP_SoftwareDelay() overhead per call
#define OVERHEAD_CYCLES_64         590 // R_BSP_SoftwareDelay() overhead per call using 64-bit ints
...
bool R_BSP_SoftwareDelay(uint32_t delay, bsp_delay_units_t units)
{
    ...
    uint32_t loop_cnt;
    uint64_t delay_cycles_64;
    uint64_t loop_cnt_64;
